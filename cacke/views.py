...
def cacke_list (request):
    ...
    if request.method == "POST":
         if(request.POST.get('Submit')):
            ...
            nam = form.get_name().lower()
   
            test = form.get_test()
            stringe = form.get_nach()
            cakes = cakes.filter(names__istartswith = str(nam)).filter(nachinka__contains = stringe).filter(testo__icontains = test).exclude(price__gte = pric[1]).filter(price__gte=pric[0])
